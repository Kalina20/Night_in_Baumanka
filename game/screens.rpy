...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            if ui_unlocked:
                textbutton _("Карта") action Show("story_map")
...
init python:
    ...
    config.overlay_screens.append("room_counter_hud")
    config.overlay_screens.append("map_corner_button")
...
## Счётчик выходов из комнат ####################################################
screen room_counter_hud():
    ...
    if ui_unlocked:
        ...
        hbox:
            ...
            $ bottle_path = "images/counter_bottle.png" if renpy.loadable("images/counter_bottle.png") else ("images/bauman_bottle.png" if renpy.loadable("images/bauman_bottle.png") else None)

            if bottle_path:
                add Transform(bottle_path, zoom=0.055, xoffset=10, yoffset=-7)
...
screen map_corner_button():

    if ui_unlocked:

        zorder 1002

        $ map_btn_path = "images/map_corner.png" if renpy.loadable("images/map_corner.png") else ("images/map_button.png" if renpy.loadable("images/map_button.png") else ("images/map.png" if renpy.loadable("images/map.png") else None))
        ...
        if map_btn_path:
            button:
                ...
                add Transform(map_btn_path, zoom=0.12)
                action Show("story_map")
        else:
            textbutton "Карта":
                xalign 0.0
                yalign 1.0
...
screen story_scene_12_note_text():
    ...
    frame:
        ...
        vbox:
            ...
            text "Ребятки, я вас не дождался и ушел в новый поход в горы! Но я хорошо знаю, что вы отличные студенты и хорошо учились). Так что спокойно забирайте свои напитки, они под преподавательским столом. Удачи!":
                xalign 0.5
                text_align 0.0
                size 30
                color "#4a3420"
...
screen story_scene_8_chain_game():
    ...
    frame:
        ...
        vbox:
            ...
            text "Перетащи элементы снизу в слоты сверху. После этого нажми Проверить.":
                xalign 0.5
                text_align 0.5
                size 24
                color "#e8e8e8"
...
screen story_map():
    ...
    fixed:
        ...
        $ map_path = "images/bauman_map.png" if renpy.loadable("images/bauman_map.png") else ("images/bauman_map.jpg" if renpy.loadable("images/bauman_map.jpg") else None)

        if map_path:

            $ map_zoom = 0.867
            $ map_w, map_h = renpy.image_size(map_path)
            $ display_w = int(map_w * map_zoom)
            $ display_h = int(map_h * map_zoom)

            frame:
                align (0.5, 0.5)
                background Solid("#2d6bff")
                padding (3, 3)
                ...
                fixed:
                    ...
                    add Transform(map_path, zoom=map_zoom, xalign=0.5, yalign=0.5)

                    for room_number, px, py in [
                        ("420", 718, 605),
                        ("421", 210, 620),
                        ("422", 765, 535),
                        ("423", 203, 515),
                        ...
                        if room_number in visited_cabinets:
                            fixed:
                                xpos int(px * map_zoom)
                                ypos int(py * map_zoom)
                                xanchor 0.5
                                yanchor 0.5
                                xsize 48
                                ysize 34
                                ...
                                add Transform(Solid("#ff2b2b"), xsize=40, ysize=2, xalign=0.5, yalign=0.5, rotate=35)
                                add Transform(Solid("#ff2b2b"), xsize=40, ysize=2, xalign=0.5, yalign=0.5, rotate=-35)
                        else:
                            textbutton room_number:
                                xpos int(px * map_zoom)
                                ypos int(py * map_zoom)
                                xanchor 0.5
                                yanchor 0.5
            ...
            vbox:
                ...
                grid 4 3:
                    ...
                    for room_number in ["420", "421", "422", "423", "424", "425", "426", "427", "428", "429", "430", "431"]:
